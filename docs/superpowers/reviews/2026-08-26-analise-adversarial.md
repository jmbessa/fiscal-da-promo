...
- **A7. Nenhum post identifica que é publicidade / link de afiliado** — CDC art. 36, guia CONAR para influenciadores (2021), política de conteúdo de marca da Meta. Um canal que se chama "Fiscal" e omite a comissão em cada post é o perfil que gera representação no CONAR. Linha fixa em todo post: "🔗 link de afiliado · #publi" (decisão de texto do dono).
  ...
  - **Estado: RESOLVIDO em 2026-08-30 (fase 5U).** A sinalização está em TODA superfície que publica. Um lugar só decide o texto — `creative.AFILIADO` ("Link de afiliado — ganho comissão, o preço é o mesmo para você") e `creative.AFILIADO_NA_ARTE` ("Link de afiliado", desenhado em caixa alta) —, no molde de `pricing.sem_cupom`; um teste troca a constante e cobra as quatro legendas. **Texto:** primeira linha do Telegram, da legenda do feed, da do carrossel e da do Reel — a única colocação que sobrevive ao corte do "mais" (~125 caracteres) e que aparece na prévia da notificação do Telegram. **Arte:** chip mono na faixa do cabeçalho do story, do feed e dos slides do carrossel (bloco centralizado na capa e no fecho), acima do card e fora do orçamento do guarda de overflow. A ÚNICA peça sem a marca é o gráfico do flagrante, que não carrega link nenhum. Recusado o `#publi` do registro original: jargão de influenciador contradiz uma conta que se chama Fiscal. O risco de perda de monetização deixa de ser "aceito" e passa a mitigado.
