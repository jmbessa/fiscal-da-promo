...
class InstagramStoryChannel(InstagramBase):
    ...
    def publish(self, post: Post) -> PublishResult:
        ...
        status_code, status = self._aguarda_container(str(creation_id))
        if status_code in STATUS_TERMINAIS:
            # O container morreu: publicar seria uma segunda chamada inútil
            # contra a cota de 100/24 h compartilhada com o feed.
            return PublishResult(False, error=f"container {status_code}: {status}")
        ...
        if not media_id:
            ...
            if status_code != "FINISHED":
                ...
                erro = (f"{erro} (container ainda {status_code or 'sem status'} "
                        f"após {self.max_polls} tentativas)")
            return PublishResult(False, error=erro)

        return PublishResult(True, str(media_id))

    def _aguarda_container(self, creation_id: str) -> tuple[str, str]:
        """Lê `status_code`/`status` do container até `max_polls` vezes.

        Devolve `(status_code, status)`. `FINISHED` para na hora; `ERROR`/
        `EXPIRED` também (não há o que esperar). Esgotadas as tentativas ainda
        em `IN_PROGRESS`, devolve isso — e o `publish` tenta publicar assim
        mesmo, que foi o que funcionou ao vivo. Falha de rede aqui vale como
        "não sei": o container existe, e não é motivo para condenar o story.
        """
        status_code, status = "", ""
        ...
        return status_code or "IN_PROGRESS", status
